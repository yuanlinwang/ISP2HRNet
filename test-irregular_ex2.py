...
def batched_predict_irregular_test(model, coordinate,value, coord, cell, bsize, ratio,hr_h,hr_w,scale):   
    ...
    # torch_hr=torch_hr.unsqueeze(0).repeat(BATCH_SIZE,1,1)
    # torch_cell=torch_cell.unsqueeze(0).repeat(BATCH_SIZE,1,1)
    with torch.no_grad():
...
def eval_psnr_irregular_test(loader, model,scale=None, data_norm=None, eval_type=None, eval_bsize=None,
              verbose=False,out_save=None,ratio=None):
    model.eval()
    # scale=scale
    # ratio=ratio
    ...
    for batch in pbar:
        ...
        if eval_bsize is None:
            ...
            pred = batched_predict_irregular_test(model, coordinate,value,
                batch['coord'], batch['cell'], eval_bsize,ratio,batch['hr_h'],batch['hr_w'],scale)
        ...
        #保存输出图像
        # out_save_name=os.path.join(out_save,str(idx)+"_s"+str(scale)+"_"+str(ratio)+".png")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config',type=str, default="configs/test-irregular/test-set5-8-rrs+sr.yaml")
    ...
    # config['test_dataset']['wrapper']['args']['ratio']=args.ratio

    # image save path
    if not os.path.exists("EX2"):
        os.mkdir("EX2")
    out_save=os.path.join("EX2", args.dataset_name)
    ...
    model_spec['args']['irregular_spec']['args']['knn_k_prime']=12 #a13_modify需要
    model = models.make(model_spec, load_sd=True).cuda()
    print(args.scale)
    psnr,ssim= eval_psnr_irregular_test(loader, model,scale=args.scale,
        data_norm=config.get('data_norm'),
        eval_type=config.get('eval_type'),
        eval_bsize=config.get('eval_bsize'),
        verbose=True,out_save=out_save,ratio=args.ratio)

    print( 'PSNR avg:%.2f'%(psnr))
    print( 'SSIM avg:%.2f'%(ssim))
    file = open('EX2.txt', 'a')
    print( '%s   --scale %.2f   ——ratio:%.2f'%(args.dataset_name,args.scale,args.ratio),file=file)
